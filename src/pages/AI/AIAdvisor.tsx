...
import { Loader2, Brain, ChevronRight, Calculator } from "lucide-react";
...
const RISK_STOCKS: Record<string, { name: string; symbol: string; type: string }[]> = {
  conservative: [
    ...
    { name: "Gold ETF", symbol: "GOLDBEES.NS", type: "etf" },
    { name: "Nifty 50 Index", symbol: "NIFTYBEES.NS", type: "etf" },
  ],
  moderate: [
  ...
  aggressive: [
    ...
    { name: "Tata Power", symbol: "TATAPOWER.NS", type: "stock" },
...
const RETURN_RATES: Record<string, number> = { conservative: 0.09, moderate: 0.12, aggressive: 0.16 };
...
export default function AIAdvisor() {
  ...
  const submitQuiz = async () => {
    ...
    setSubmitting(true);
    const totalScore = answers.reduce((s, a) => s + (a + 1), 0); // 1-5 per question
    const avgScore = totalScore / answers.length;
    const riskScore = Math.round(avgScore * 2); // scale to 1-10
    ...
    setRiskProfile({ risk_score: riskScore, risk_category: riskCategory });
  ...
  const recommendations = RISK_STOCKS[category] ?? [];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground">Get personalised investment recommendations</p>
      </div>
      ...
      {riskProfile && (
        <Card className="glass-card">
          <CardHeader>
            <CardTitle className="text-lg">Recommendations for {category} investors</CardTitle>
          </CardHeader>
          <CardContent>
              ...
              {recommendations.map((r, i) => (
                <Card key={i} className="bg-muted/30 border-border/50">
                  <CardContent className="p-4 space-y-2">
                    ...
                    <p className="text-sm font-mono-number text-muted-foreground">{r.symbol}</p>
                    <Badge className="bg-success/20 text-success border-success/30">Suggested</Badge>
                  </CardContent>
                </Card>
              ))}
            </div>
